...
def load_data() -> pd.DataFrame:
    ...
    if "kpi_reliability_flag" not in df.columns:
        df["kpi_reliability_flag"] = "UNKNOWN"
    if "calibration_policy" not in df.columns:
        df["calibration_policy"] = "legacy_calibration"
    df["kpi_reliability_flag"] = df["kpi_reliability_flag"].fillna("UNKNOWN")
    df["calibration_policy"] = df["calibration_policy"].fillna("legacy_calibration")
...
def series_from_rows(rows: pd.DataFrame, history_lookup: pd.DataFrame, keys: list[str]) -> dict:
    ...
    return {
        ...
        "kpi_reliability_flag": str(latest.get("kpi_reliability_flag", "UNKNOWN")),
        "calibration_policy": str(latest.get("calibration_policy", "legacy_calibration")),
        "risk_rows": risk_rows,
...
def aggregate_account(rows: pd.DataFrame) -> pd.DataFrame:
    grouped = rows.groupby(["account_id", "date"], as_index=False).agg(
        ...
        kpi_reliability_flag=("kpi_reliability_flag", "last"),
        calibration_policy=("calibration_policy", "last"),
    )
